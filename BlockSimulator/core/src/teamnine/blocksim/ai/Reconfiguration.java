...
public class Reconfiguration 
{
	...
	/**
	 * Constructor for reconfiguration algorithm
	 * Checks if the number of robot blocks == number of target blocks
	 * Not yet implemented: check if the ID's correspond
	 * @param robot list with all robot blocks in the neighborhood (how do we know where they are exactly)
	...
	/**
	 * 1) Sort the targets in a specific way (how?) to avoid deadlocks
	 * 2) Figure out which robot-blocks do have to go to a specific target, and which ones doesn't 
	 */
	...
	/**
	 * Stores the target-blocks in buckets, every bucket representing level with a set of target positions that should be filled first, before the positions will be filled 
	 * (2x2x2 target, origin at left-top-on floor, level-matrix: floor[0,1;1,2] floor+1[5,4;4,3]
//TODO: CHECK MATRIX ABOVE
	 * This does not have to be the most efficient way, since other targets could be reached as well and blocks for the other positions may be at a shorter path.
	 * Works in linear time
	 */
	private void sortTarget()
	{
		
		//Define origin for layer 0: point where robot blocks enter the goal region
		...
		{
			targetWidth = (int) Math.max(targetWidth, Math.abs(target.get(i).getPosition().x-layer0Origin.x+1)); // +1: if there was only one block, the abs distance would be 0, but should be 1 
			targetLength = (int) Math.max(targetLength, Math.abs(target.get(i).getPosition().z-layer0Origin.z+1));
			targetHeight = (int) Math.max(targetHeight, target.get(i).getPosition().y-1); //robot blocks on the floor are not taken into account for height; so -1
		}
		
		//The size of these arraylists depend on the dimension of the target region (which is seen as one whole cube covering the region		
		...
		for(int i=0; i<target.size(); i++)
		{
			...
			{
				//the #bucket depends on the absolute distance to the origin point (so: (x+1 & z)-->1, (x & z+1)-->1, (x+1 & z+1)-->2
	...
	private void findTarget()
	{
		for(int i=0; i<robot.size(); i++)
		{
			try
			{
				...
				// At the end of the for-loop it may occur that the target is still unspecified, then there wasn't a target block assigned to it
			}
			catch(NullPointerException e) {} // specifiedTarget stays uninitialized, i.e. there is no target specified for this robotBlock
		}
	}
	
	/**
	 * 1) Get last a targetBlock in the current level
	 * 2) Check if the targetBlock has an ID
	 * 2.1) In case not, check if the last RobotBlock in the chain is not determined to be at another position (method findTarget should have been run before)
	 * 2.2) In case it did, check if the last RobotBlock in the chain has this block as an target (method findTarget should have been run before)
	 * 3.1) IF the last Robot block in the chain meets the criteria above, THEN move it to the target position
	 * 3.2) IF NOT, THEN move it to the other chain
	 * 4) Do this WHILE not all target blocks of the same level have been filled up > this might not be the most efficient (since blocks are move between chains), however it is the best way to avoid dead-locks
	 */
	private void start()
	{
		//Get last robotblock from 'chain'
		//Check if it can be moved to one of the next targets (i.e. target ID matches 
		//IF so THEN move the block to that position
		//IF not THEN move the block to another chain
		
		int currentLevel;
		
		//TODO: Assuming that robot.get(0) gives the last robot block in the chain, which is able to move
			...
			if(sortedTargets[currentLevel].size()!=0)
			{
				...
				while (sortedTargets[currentLevel].size()!=0)
				{
					...
					for(int j=0; j<sortedTargets[currentLevel].size(); j++)
					{
						try
						{
							sortedTargets[currentLevel].get(j).getID(); //The ID for this Target Position IS specified
							try
							{
						...
						catch (NullPointerException e) //The ID WAS NOT specified for the target block, so if the robot block doesn't have an assigned target, it can move into it 
						{
							try
							{
						...
						if(DEBUG) cntr++;
						if(DEBUG&&cntr>robot.size()) {System.out.println("//RECONFIG "+"All robot blocks have been tried, but no one was able to move to the target position since ID's didn't match");}
					}
				}
			}
		}
			...
			for(int j=0; j<robot.size(); j++){
				...
				if(targetPosition.x==robotPosition.y && targetPosition.y==robotPosition.y && targetPosition.z==robotPosition.z)
				{
					...
					try{
						...
						//Check if ID is specified for robot and matches with target
						try
						{
