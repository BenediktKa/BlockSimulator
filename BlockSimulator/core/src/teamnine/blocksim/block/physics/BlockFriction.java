...
public class BlockFriction extends BlockCollision
{
	...
	public BlockFriction(Vector3 position, BlockType type)
	{
		super(position, type);
	...
	{
		Block blockUnder = blockListController.getBlockAtPointIgnoreType(new Vector3(Math.round(position.x), Math.round(position.y) - 1, Math.round(position.z)), BlockType.Goal);
