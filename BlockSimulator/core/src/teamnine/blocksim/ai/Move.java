...
public class Move
{
	...
	public void decideMove(Vector3 v)
	{
		...
		float targetZ = v.z;
		// finds distance from the block to the target assigns the value to the
		// block
	...
	// orders the block based on their distance to the target using bucket sort.
	...
	// once the order has been made, the movements will go to try and place the
	// block as close to the first step of
	// the path as possible all the way to the target
	public void moving(RobotBlock b, Vector3 v)
	{

		// System.out.println("moving startblock: "+b+" b.vector:
		// "+b.getPosition()+" "+" t.vector: "+v);
		Vector3 bestMovement = new Vector3(0, 0, 0);
		boolean targetReached = false;

		// runs until target has been reached or no further movements are
		// possible
		while (!targetReached)
		{
			try
			...
			for (int k = 0; k < robots.size(); k++)
			{
				...
				if (b.getPosition().x + 1 == robots.get(k).getPosition().x && robots.get(k).getPosition().y == b.getPosition().y && robots.get(k).getPosition().z == b.getPosition().z || b.getPosition().x - 1 == robots.get(k).getPosition().x && robots.get(k).getPosition().y == b.getPosition().y && robots.get(k).getPosition().z == b.getPosition().z || b.getPosition().x == robots.get(k).getPosition().x && robots.get(k).getPosition().y == b.getPosition().y && robots.get(k).getPosition().z == b.getPosition().z + 1 || b.getPosition().x == robots.get(k).getPosition().x && robots.get(k).getPosition().y == b.getPosition().y && robots.get(k).getPosition().z == b.getPosition().z - 1 || b.getPosition().z == robots.get(k).getPosition().z && robots.get(k).getPosition().y == b.getPosition().y - 1 && b.getPosition().x == robots.get(k).getPosition().x)
				{
					safe = true;
				}
			...
			if (safe)
			{
				if (v.x == b.getPosition().x + 1 && v.z == b.getPosition().z)
				{
					...
					{
						// System.out.println(robots.get(l).getPosition()+" ,
						// "+b.getPosition());
		...
		for (int i = 0; i < pm.size(); i++)
		{
			for (int j = 0; j < robots.size(); j++)
			{
				if (pm.get(i).x == robots.get(j).getPosition().x && pm.get(i).y == robots.get(j).getPosition().y && pm.get(i).z == robots.get(j).getPosition().z)
				{
					...
					pm.remove(i);
					// System.out.println("size "+possibleMovements.size()+" i
					// "+i);
					if (pm.size() <= i)
						break;
				}
